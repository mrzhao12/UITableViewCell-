...
class LLJBezierPath {
    ...
    class func drawRoundedRect(rect: CGRect, byRoundingCorners: [UIRectCorner], cornerRadius: [CGFloat]) -> UIBezierPath {
        ...
        if byRoundingCorners.contains(UIRectCorner.topLeft) {        //处理第一个角
        ...
        if byRoundingCorners.contains(UIRectCorner.topRight) {    //处理第二个角
        ...
        if byRoundingCorners.contains(UIRectCorner.bottomRight) {         //处理第三个角
        ...
        if byRoundingCorners.contains(UIRectCorner.bottomLeft) {    //处理第四个角
        ...
        if byRoundingCorners.contains(UIRectCorner.topLeft) {        //再次处理第一个角
            let position = byRoundingCorners.firstIndex(of: UIRectCorner.topLeft)
